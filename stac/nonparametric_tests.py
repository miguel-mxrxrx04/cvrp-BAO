...
def friedman_aligned_ranks_test(*args):
    ...
    r_i = [sum(case) for case in aligned_ranks]
    r_j = [sum([case[j] for case in aligned_ranks]) for j in range(k)]
    T = (k-1) * (sum(v**2 for v in r_j) - (k*n**2/4.) * (k*n+1)**2) / float(((k*n*(k*n+1)*(2*k*n+1))/6.) - (1./float(k))*sum(v**2 for v in r_i))
...
def quade_test(*args):
    ...
    Sj = [sum(row[j] for row in S) for j in range(k)]
    Wj = [sum(row[j] for row in W) for j in range(k)]
    
    rankings_avg = [w / (n*(n+1)/2.) for w in Wj]
    rankings_cmp = [r/np.sqrt(k*(k+1)*(2*n+1)*(k-1)/(18.*n*(n+1))) for r in rankings_avg]

    A = sum(S[i][j]**2 for i in range(n) for j in range(k))
    B = sum(s**2 for s in Sj)/float(n)
    F = (n-1)*B/(A-B)
